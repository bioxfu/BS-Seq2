...
my %chr_len = ("Chr1"=>30427671, "Chr2"=>19698289, "Chr3"=>23459830, "Chr4"=>18585056, "Chr5"=>26975502, "ChrC"=>154478, "ChrM"=>366924);
...
while($l_wt = <WT>, $l_mut = <MUT>){
	...
	#print STDERR "checking...".$chr."\t".$pos."\t".$type."\t".$dep_wt."\t".$dep_mut."\t".$mC_wt."\t".$mC_mut."\n";
...
#				print STDERR "+++++++\n";
#				print STDERR "$type\t$chr\t$i\n";
#				print STDERR "$dep_num_wt{$type}->{$chr}->[$i]\n";
#				print STDERR "$dep_num_mut{$type}->{$chr}->[$i]\n";
#				print STDERR "$mC_num_wt{$type}->{$chr}->[$i]\n";
#				print STDERR "$mC_num_mut{$type}->{$chr}->[$i]\n";
			}
		}
	}
}
...
foreach my $chr(sort keys %chr_len){
	...
	for (my $i = 0; $i <= $bin_last_index{$chr}; $i++){
		...
		#	my $ref_num = 0; 
			...
			my $p_val = 1;
			#print STDERR "checking...\t".$last."\t".$i."\t".$chr."\t".$start."\t".$end."\t".$n11."\t".$n1p."\t".$np1."\t".$npp."\n";
